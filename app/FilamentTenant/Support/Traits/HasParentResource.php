...
//        if ($parent = (request()->route('ownerRecord') ?? request()->input('ownerRecord'))) {
//            $parentResource = $this->getParentResource();
//
//            $this->ownerRecord = $parentResource::resolveRecordRouteBinding($parent);
//
//            if (!$this->ownerRecord) {
...
//
//        return $query->where($this->getParentRelationshipKey(), $this->ownerRecord->getKey());
//    }
//
...
//        return $this->relationshipKey ?? $this->ownerRecord?->getForeignKey();
//    }
//
...
//            $parentResource::getRecordTitle($this->ownerRecord),
//            $parentResource::getUrl(name: $this->getChildPageNamePrefix() . '.index', parameters: ['ownerRecord' => $this->ownerRecord]) => $resource::getBreadCrumb(),
//        ];
//
