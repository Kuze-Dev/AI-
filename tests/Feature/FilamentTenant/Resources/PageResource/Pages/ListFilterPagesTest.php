...
it('can filter pages by blueprint', function () {
    ...
    livewire(ListPages::class)
        ...
        ->filterTable('blueprint', $blueprintId)
        ->assertCanSeeTableRecords($pages->where('blueprint_id', $blueprintId))
        ->assertCanNotSeeTableRecords($pages->where('blueprint_id', '!=', $blueprintId))
        ->assertOk();
});
